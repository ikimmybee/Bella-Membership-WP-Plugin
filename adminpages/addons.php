<?php
	...
	?>
	<h2><?php _e('Add Ons', 'bella-membership-plugin' ); ?></h2>
	...
	<p>
		<?php printf(__('Last checked on %s at %s.', 'bella-membership-plugin' ), date_i18n(get_option('date_format'), $addons_timestamp), date_i18n(get_option('time_format'), $addons_timestamp));?> &nbsp;	
		<a class="button" href="<?php echo admin_url("admin.php?page=pmpro-addons&force-check=1&plugin_status=" . $status);?>"><?php _e('Check Again', 'bella-membership-plugin' ); ?></a>
	</p>

	<ul class="subsubsub">
		<li class="all"><a href="admin.php?page=pmpro-addons&plugin_status=all" <?php if(empty($status) || $status == "all") { ?>class="current"<?php } ?>><?php _e('All', 'bella-membership-plugin' ); ?> <span class="count">(<?php echo count($addons);?>)</span></a> |</li>
		<li class="active"><a href="admin.php?page=pmpro-addons&plugin_status=active" <?php if($status == "active") { ?>class="current"<?php } ?>><?php _e('Active', 'bella-membership-plugin' ); ?> <span class="count">(<?php echo count($addons_active);?>)</span></a> |</li>
		<li class="inactive"><a href="admin.php?page=pmpro-addons&plugin_status=inactive" <?php if($status == "inactive") { ?>class="current"<?php } ?>><?php _e('Inactive', 'bella-membership-plugin' ); ?> <span class="count">(<?php echo count($addons_inactive);?>)</span></a> |</li>
		<li class="update"><a href="admin.php?page=pmpro-addons&plugin_status=update" <?php if($status == "update") { ?>class="current"<?php } ?>><?php _e('Update Available', 'bella-membership-plugin' ); ?> <span class="count">(<?php echo count($addons_update);?>)</span></a> |</li>
		<li class="uninstalled"><a href="admin.php?page=pmpro-addons&plugin_status=uninstalled" <?php if($status == "uninstalled") { ?>class="current"<?php } ?>><?php _e('Not Installed', 'bella-membership-plugin' ); ?> <span class="count">(<?php echo count($addons_uninstalled);?>)</span></a></li>
	</ul>
	...
	<tr>
		...
		<th scope="col" id="name" class="manage-column column-name" style=""><?php _e('Add On Name', 'bella-membership-plugin' ); ?></th>
		<th scope="col" id="type" class="manage-column column-type" style=""><?php _e('Type', 'bella-membership-plugin' ); ?></th>
		<th scope="col" id="description" class="manage-column column-description" style=""><?php _e('Description', 'bella-membership-plugin' ); ?></th>		
	</tr>
	</thead>
	<tbody id="the-list">
		<?php
			...
			<tr>
				<td></td>
				<td colspan="3"><p><?php _e('No Add Ons found.', 'bella-membership-plugin' ); ?></p></td>	
			</tr>
			<?php
			}
					...
					<td class="plugin-title">
						...
						<?php
							...
							if($context === 'uninstalled inactive')
							{
								...
								{
									//wordpress.org
									$actions['install'] = '<span class="install"><a href="' . wp_nonce_url(self_admin_url('update.php?action=install-plugin&plugin=' . $plugin_data['Slug']), 'install-plugin_' . $plugin_data['Slug']) . '">' . __('Install Now', 'bella-membership-plugin' ) . '</a></span>';
								}
								elseif($plugin_data['License'] == 'free')
								{
									//free
									$actions['install'] = '<span class="install"><a href="' . wp_nonce_url(self_admin_url('update.php?action=install-plugin&plugin=' . $plugin_data['Slug']), 'install-plugin_' . $plugin_data['Slug']) . '">' . __('Install Now', 'bella-membership-plugin' ) . '</a></span>';
									$actions['download'] = '<span class="download"><a target="_blank" href="' . $plugin_data['Download'] . '?key=' . $pmpro_license_key . '">' . __('Download', 'bella-membership-plugin' ) . '</a></span>';
								}
								elseif(empty($pmpro_license_key))
								{
									//no key
									$actions['settings'] = '<span class="settings"><a href="' . admin_url('options-general.php?page=pmpro_license_settings') . '">' . __('Update License', 'bella-membership-plugin' ) . '</a></span>';
									$actions['download'] = '<span class="download"><a target="_blank" href="' . $plugin_data['PluginURI'] . '">' . __('Download', 'bella-membership-plugin' ) . '</a></span>';
								}
					...
					<td class="column-type">
						<?php
							if($addon['License'] == 'free')
								_e("PMPro Free", 'bella-membership-plugin' );
							elseif($addon['License'] == 'core')
								_e("PMPro Core", 'bella-membership-plugin' );
							elseif($addon['License'] == 'plus')
								_e("PMPro Plus", 'bella-membership-plugin' );
							elseif($addon['License'] == 'wordpress.org')
								_e("WordPress.org", 'bella-membership-plugin' );
							else
								_e("N/A", 'bella-membership-plugin' );
						?>
					</td>
