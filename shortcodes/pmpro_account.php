	...
	<div id="pmpro_account">		
		<?php if(in_array('membership', $sections) || in_array('memberships', $sections)) { ?>
			<div id="pmpro_account-membership" class="pmpro_box">
				...
				<h3><?php _e("My Memberships", 'bella-membership-plugin' );?></h3>
				<table width="100%" cellpadding="0" cellspacing="0" border="0">
					<thead>
						<tr>
							<th><?php _e("Level", 'bella-membership-plugin' );?></th>
							<th><?php _e("Billing", 'bella-membership-plugin' ); ?></th>
							...
							<td class="pmpro_account-membership-levelname">
								...
								<div class="pmpro_actionlinks">
									...
									<?php if( array_key_exists($level->id, $pmpro_levels) && pmpro_isLevelExpiringSoon( $level ) ) { ?>
										<a href="<?php echo pmpro_url("checkout", "?level=" . $level->id, "https")?>"><?php _e("Renew", 'bella-membership-plugin' );?></a>
									<?php } ?>
									...
									<?php if((isset($ssorder->status) && $ssorder->status == "success") && (isset($ssorder->gateway) && in_array($ssorder->gateway, array("authorizenet", "paypal", "stripe", "braintree", "payflow", "cybersource"))) && pmpro_isLevelRecurring($level)) { ?>
										<a href="<?php echo pmpro_url("billing", "", "https")?>"><?php _e("Update Billing Info", 'bella-membership-plugin' ); ?></a>
									<?php } ?>
									
									<?php 
										//To do: Only show CHANGE link if this level is in a group that has upgrade/downgrade rules
										if(count($pmpro_levels) > 1 && !defined("PMPRO_DEFAULT_LEVEL")) { ?>
										<a href="<?php echo pmpro_url("levels")?>"><?php _e("Change", 'bella-membership-plugin' );?></a>
									<?php } ?>
									<a href="<?php echo pmpro_url("cancel", "?levelstocancel=" . $level->id)?>"><?php _e("Cancel", 'bella-membership-plugin' );?></a>
				...
				<div class="pmpro_actionlinks">
					<a href="<?php echo pmpro_url("levels")?>"><?php _e("View all Membership Options", 'bella-membership-plugin' );?></a>
				</div>
		...
		<?php if(in_array('profile', $sections)) { ?>
			<div id="pmpro_account-profile" class="pmpro_box">	
				...
				<h3><?php _e("My Account", 'bella-membership-plugin' );?></h3>
				...
				<ul>
					...
					<li><strong><?php _e("Username", 'bella-membership-plugin' );?>:</strong> <?php echo $current_user->user_login?></li>
					<li><strong><?php _e("Email", 'bella-membership-plugin' );?>:</strong> <?php echo $current_user->user_email?></li>
					<?php do_action('pmpro_account_bullets_bottom');?>
				</ul>
				<div class="pmpro_actionlinks">
					<a href="<?php echo admin_url('profile.php')?>"><?php _e("Edit Profile", 'bella-membership-plugin' );?></a>
					<a href="<?php echo admin_url('profile.php')?>"><?php _e('Change Password', 'bella-membership-plugin' );?></a>
				</div>
		...
		<div id="pmpro_account-invoices" class="pmpro_box">
			<h3><?php _e("Past Invoices", 'bella-membership-plugin' );?></h3>
			<table width="100%" cellpadding="0" cellspacing="0" border="0">
				<thead>
					<tr>
						<th><?php _e("Date", 'bella-membership-plugin' ); ?></th>
						<th><?php _e("Level", 'bella-membership-plugin' ); ?></th>
						<th><?php _e("Amount", 'bella-membership-plugin' ); ?></th>
					</tr>
				</thead>
				<tbody>
				<?php 
					...
					{ 
						...
						<tr id="pmpro_account-invoice-<?php echo $invoice->code; ?>">
							...
							<td><?php if(!empty($invoice->membership_level)) echo $invoice->membership_level->name; else echo __("N/A", 'bella-membership-plugin' );?></td>
			...
			<?php if($count == 6) { ?>
				<div class="pmpro_actionlinks"><a href="<?php echo pmpro_url("invoice"); ?>"><?php _e("View All Invoices", 'bella-membership-plugin' );?></a></div>
			<?php } ?>
		...
		<div id="pmpro_account-links" class="pmpro_box">
			<h3><?php _e("Member Links", 'bella-membership-plugin' );?></h3>
			<ul>
				<?php 
